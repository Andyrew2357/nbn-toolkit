...
def preset_label(preset, label=""):
    match preset:
        case 'V':
            return r'[{}]'.format(VOLT_UNIT)
        case 'Vb':
            return r'$V_{{bg}}$ [{}]'.format(VOLT_UNIT)
        case 'Vt':
            return r'$V_{{tg}}$ [{}]'.format(VOLT_UNIT)
        case 'n':
            return r'n [{}]'.format(DENS_UNIT)
        case 'D':
            return r'$D/\epsilon_{{\circ}}$ [{}]'.format(DISP_UNIT)
        case 'B':
            return r'B [{}]'.format(BFLD_UNIT)
        case 'I':
            return r'[{}]'.format(CURR_UNIT)
        case 'Ix':
            return r'$I_{{x}}$ [{}]'.format(CURR_UNIT)
        case 'Iy':
            return r'$I_{{y}}$ [{}]'.format(CURR_UNIT)
        case 'R':
            return r'R [{}]'.format(RSST_UNIT)
        case 'dR':
            return r'$(dI/dV)^{{-1}}$ [{}]'.format(RSST_UNIT)
        case 'G':
            return r'G [{}]'.format(COND_UNIT)
        case 'dG':
            return r'dI/dV [{}]'.format(COND_UNIT)
        case 'C':
            return r'$C/\Delta A$ [{}]'.format(CAPT_UNIT)
        case '-':
            return label
        case _:
            raise SyntaxError(f"""{preset} is not a valid preset. Consult 
